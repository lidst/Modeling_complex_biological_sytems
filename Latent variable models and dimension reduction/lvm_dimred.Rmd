...
png("bottomly_heatmap_clustered.png",height=7,width=7)
heatmap.2(edata,
...
```{r}
pdf("Stadnik_problem1.pdf", height=7, width=7)  # Use pdf() instead of png()
heatmap.2(edata,
          main = "Bottomly et al. Clustered & Scaled", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
          col=my_palette,       # use on color palette defined earlier 
          dendrogram = "column", # Show dendrogram only on columns
          scale = "column",      # Scale across columns
...
```{r}
ggsave("Stadnik_problem2.pdf", ggplot(PC) + geom_point(aes(x=V3, y=V2, col=as.factor(strain))), width = 6, height = 4, dpi = 300)
```
...
```{r}
left_singular_vectors <- data.table(svd.out$u[, 1:5])
colnames(left_singular_vectors) <- paste0("V", 1:5)
left_singular_vectors$Gene <- 1:nrow(left_singular_vectors)

# Melt the data for ggplot
left_singular_vectors_melt <- melt(left_singular_vectors, id.vars = "Gene")

# Plot violin plots
p4 <- ggplot(left_singular_vectors_melt, aes(x=variable, y=value)) + 
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75), fill="orange", size = 0.1) +
  geom_jitter(width=0.1, alpha=0.5, size = 0.4) +
  labs(title="Top 5 left singular vectors", x="Singular Vector", y="Value") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))  
ggsave("Stadnik_problem4.pdf", p4, width=6, height=4, dpi=300, bg ="white")
```
...
library(ggplot2)
set.seed(1)
# Perform k-means clustering on the original data with k=5
...
set.seed(1)
tsne_out <- Rtsne(edata, pca=FALSE, perplexity=60)
tsne_data <- data.table(tsne_out$Y)
tsne_data$Cluster <- cluster_labels

# Plot the t-SNE projection with clusters
p5 <- ggplot(tsne_data, aes(x=V1, y=V2, col=Cluster)) +
  geom_point(alpha=0.6) +
  labs(title="t-SNE Projection with K-means Clustering (k=5)",
       x="t-SNE 1", y="t-SNE 2", col="Cluster") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))

ggsave("Stadnik_problem5.pdf", p5, width=6, height=4, dpi=300, bg="white")
```
