...
"
pdf("Stadnik_problem1.pdf", width = 7, height = 10)  # Increase height to fit both text and table
...
# Histogram for ComBat p-values
ggplot(modbattcombat_tidy %>% filter(term == "as.factor(pdata$strain)DBA/2J")) + 
  geom_histogram(aes(x=p.value), bins=100, fill="steelblue") + 
  ggtitle("ComBat p-values")

# Histogram for SVA p-values
ggplot(mod_sva_tidy_bott %>% filter(term == "as.factor(pdata$strain)DBA/2J")) + 
  geom_histogram(aes(x=p.value), bins=100, fill="darkorange") + 
