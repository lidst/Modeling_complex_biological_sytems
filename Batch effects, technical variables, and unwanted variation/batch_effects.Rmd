...
batch_table <- as.data.frame.matrix(table(pheno$batch, pheno$outcome))
batch_table$Total_samples <- rowSums(batch_table)
batch_table <- cbind(Batch = rownames(batch_table), batch_table)
batch_matrix <- as.matrix(batch_table)

explanation_text <- "
Some batches (e.g., Batch 3) contain only one outcome type, which may indicate potential batch effects.\n Batch 2 has a mix of 'mTCC,' 'Normal,' and 'sTCC-CIS' samples, whereas Batch 4 and 5 are more specific\n to certain conditions. The imbalance in distribution, combined with the significant difference\n in the number of samples between batches (ranging from 4 to 19 samples), can introduce confounding effects.\n The differences between groups may be due to batch effects rather than true biological differences.\nThis variation in sample sizes across batches could impact downstream analyses, such as differential\n expression studies, leading to misleading results.\n
"

pdf("batch_effects_summary.pdf", width = 7, height = 10)  # Increase height to fit both text and table
grid.text(explanation_text, x = 0.5, y = 0.95, just = "top", gp = gpar(fontsize = 10, lineheight = 1.2))
grid.table(batch_matrix, rows = NULL, vp = viewport(y = 0.45))  # Position the table lower
dev.off() 
```
...
edata_sorted <- edata[, order(pheno$batch)] 
combat_edata_sorted <- combat_edata[, order(pheno$batch)]

png("Stadnik_problem2_1.png", height=700, width=700)
...
png("Stadnik_problem2_2.png", height=700, width=700)
...
```{r}
cor_mat <- cor(combat_edata, method = "pearson")  

sample_labels <- paste(pheno$cancer, pheno$outcome)
batch_colors <- c("1" = "green", "2" = "blue", "3" = "orange", "4" = "purple", "5" = "red")
label_colors <- batch_colors[as.character(pheno$batch)]

library(gplots)
library(RColorBrewer)
my_palette <- colorRampPalette(c("blue", "white", "darkred"))(n = 299)

png("Stadnik_problem3.png", height=700, width=700)
heatmap.2(cor_mat,
          main = "Sample Correlations (Pearson)",
          trace = "none",
          density.info = "none",
          col = my_palette,
          margins = c(12, 12),
          ...
          colCol = label_colors,
          colRow = label_colors)
legend("topright", legend = paste("Batch", 1:5), fill = batch_colors, border = NA, bty = "n", cex = 0.8)
dev.off()
```
...
expdata <- as.matrix(exprs(bottomly.eset))
expdata <- expdata[rowMeans(expdata) > 10, ]
expdata <- log2(as.matrix(expdata) + 1)

pdata=pData(bottomly.eset)
# Linear model for uncorrected gene expression data
pdata$strain <- relevel(pdata$strain, ref = "C57BL/6J")  
mod_bott = lm(t(expdata) ~ as.factor(pdata$strain) + as.factor(pdata$experiment.number))

names(mod_bott)
dim(mod_bott$coefficients)
rownames(mod_bott$coefficients)

mod_bott_tidy <- tidy(mod_bott)

# Linear model for corrected gene expression data
experiment_number = pdata$experiment.number
combat_expdata = ComBat(dat=expdata, batch=pdata$experiment.number, mod=model.matrix(~1, data=pdata), par.prior=TRUE, prior.plots=TRUE)

modbattcombat = lm(t(combat_expdata) ~ as.factor(pdata$strain))
modbattcombat_tidy <- tidy(modbattcombat)

# Visualisation
est_compare_bott <- tibble(
  LinearModel_bottomly = mod_bott_tidy %>% filter(term == "as.factor(pdata$strain)DBA/2J") %>% select("estimate") %>% unlist,
  ComBat_bottomly = modbattcombat_tidy %>% filter(term == "as.factor(pdata$strain)DBA/2J") %>% select("estimate") %>% unlist)

ggplot(est_compare_bott, aes(x=LinearModel_bottomly, y=ComBat_bottomly)) +
  geom_point(col="darkgrey", alpha=.5, size=.5) + 
  geom_abline(intercept=0, slope=1, col="darkred") + 
  geom_smooth(method = "lm", se = TRUE) + 
  theme_bw()

ggplot(mod_bott_tidy %>% filter(term == "as.factor(pdata$strain)DBA/2J")) + 
  geom_histogram(aes(x=p.value), bins = 100, fill="darkorange") +
  ggtitle("Histogram of p-values: Uncorrected Model")

ggplot(modbattcombat_tidy %>% filter(term == "as.factor(pdata$strain)DBA/2J")) + 
  geom_histogram(aes(x=p.value), bins = 100, fill="darkorange") +
  ggtitle("Histogram of p-values: ComBat-Corrected Model")

est_compare_bott <- tibble(
  LinearModel_bottomly = mod_bott_tidy %>% filter(term == "as.factor(pdata$strain)DBA/2J") %>% select("estimate") %>% unlist,
  ComBat_bottomly = modbattcombat_tidy %>% filter(term == "as.factor(pdata$strain)DBA/2J") %>% select("estimate") %>% unlist)

plot1 <- ggplot(est_compare_bott, aes(x = LinearModel_bottomly, y = ComBat_bottomly)) +
  geom_point(col = "darkgrey", alpha = .5, size = .5) + 
  geom_abline(intercept = 0, slope = 1, col = "darkred") + 
  geom_smooth(method = "lm", se = TRUE) + 
  theme_bw()

ggsave("Stadnik_problem4_1.png", plot = plot1)

plot2 <- ggplot(mod_bott_tidy %>% filter(term == "as.factor(pdata$strain)DBA/2J")) + 
  geom_histogram(aes(x = p.value), bins = 100, fill = "darkorange") +
  ggtitle("Histogram of p-values: Uncorrected Model")

ggsave("Stadnik_problem4_2.png", plot = plot2)

plot3 <- ggplot(modbattcombat_tidy %>% filter(term == "as.factor(pdata$strain)DBA/2J")) + 
  geom_histogram(aes(x = p.value), bins = 100, fill = "darkorange") +
  ggtitle("Histogram of p-values: ComBat-Corrected Model")

ggsave("Stadnik_problem4_3.png", plot = plot3)
...
set.seed(1)
rnorm(1)
num.sv(expdata,mod,method="be")
num.sv(expdata,mod,method="leek")

mod_bott = model.matrix(~ as.factor(pdata$strain), data=pdata)
mod0_bott = model.matrix(~1, data=pdata)
sva_output_bott = sva(expdata, mod_bott, mod0_bott, n.sv=num.sv(expdata,mod_bott,method="leek"))


mod_sva_bott = lm(t(expdata) ~ as.factor(pdata$strain) + sva_output_bott$sv)
mod_sva_tidy_bott = tidy(mod_sva_bott)

# Visualisation
est_compare <- tibble(
  ComBat = modbattcombat_tidy %>%
    filter(term == "as.factor(pdata$strain)DBA/2J") %>% pull(estimate),
  SVA = mod_sva_tidy_bott %>%
    filter(term == "as.factor(pdata$strain)DBA/2J") %>% pull(estimate)
)

plot1 <- ggplot(est_compare, aes(x = ComBat, y = SVA)) +
  geom_point(color = "grey30", alpha = 0.5, size = 0.7) +
  geom_abline(slope = 1, intercept = 0, color = "darkred") +
  geom_smooth(method = "lm", se = TRUE) +
  theme_bw() +
  labs(title = "Comparison of Coefficients: ComBat vs SVA")

ggsave("Stadnik_problem5_1.png", plot = plot1)

plot2 <- ggplot(modbattcombat_tidy %>% filter(term == "as.factor(pdata$strain)DBA/2J")) + 
  geom_histogram(aes(x = p.value), bins = 100, fill = "steelblue") + 
  ggtitle("ComBat p-values")

ggsave("Stadnik_problem5_2.png", plot = plot2)

plot3 <- ggplot(mod_sva_tidy_bott %>% filter(term == "as.factor(pdata$strain)DBA/2J")) + 
  geom_histogram(aes(x = p.value), bins = 100, fill = "darkorange") + 
  ggtitle("SVA p-values")

ggsave("Stadnik_problem5_3.png", plot = plot3)
